      ...
      try {
        ...
        if (res.success) {
          ...
          if (initialForm.startDate && res.booking.packageDetails?.duration) {
            initialForm.endDate = calculateEndDate(initialForm.startDate, res.booking.packageDetails.duration);
          }
        ...
        <div className="container mx-auto px-6">
          ...
          <div className="max-w-6xl mx-auto">
            <div className="grid lg:grid-cols-2 gap-8">
                ...
                <div className="space-y-6">
                  ...
                  <div className="relative h-48 bg-gradient-to-br from-green-600/20 to-green-400/20 rounded-lg overflow-hidden mb-4">
                    {packageData.image?.url ? (
                      <img src={packageData.image.url} alt={packageData.title} className="w-full h-full object-cover" />
                    ) : (
                    ...
                    {packageData.isPopular && (
                      <div className="absolute top-4 left-4">
                        <span className="bg-green-500 text-white px-3 py-1 rounded-full text-xs font-abeze font-bold">Popular</span>
                      </div>
                    )}
                  </div>
                  {/* Gallery/Photos (supports photos, gallery, images) */}
                  {(() => {
                    const galleryArr = packageData.photos?.length ? packageData.photos
                      : packageData.gallery?.length ? packageData.gallery
                      : packageData.images?.length ? packageData.images
                      : [];
                    return galleryArr.length > 0 ? (
                      <div className="mb-4">
                        <h4 className="text-green-400 font-abeze font-medium text-base mb-2">Gallery</h4>
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                          {galleryArr.map((photo, idx) => (
                            <img key={idx} src={photo.url || photo} alt={`Photo ${idx + 1}`} className="w-full h-24 object-cover rounded-lg border border-white/20" />
                          ))}
                        </div>
                      </div>
                    ) : null;
              ...
              {/* Booking Update Form */}
              <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-6 border border-white/20">
                <h2 className="text-2xl font-abeze font-bold text-white mb-6">Update Booking Details</h2>
                <form onSubmit={handleSubmit} className="space-y-6">
                  <div className="grid md:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-white font-abeze font-medium mb-2">Start Date *</label>
                      <input type="date" name="startDate" value={form.startDate || ''} onChange={handleInputChange} min={new Date().toISOString().split('T')[0]} className={`w-full bg-white/10 border rounded-lg px-4 py-3 text-white font-abeze placeholder-gray-400 focus:outline-none transition-colors ${errors.startDate ? 'border-red-400' : 'border-white/20 focus:border-green-400'}`} />
                      {errors.startDate && <p className="text-red-400 text-sm mt-1 font-abeze">{errors.startDate}</p>}
                    </div>
                    <div>
                      <label className="block text-white font-abeze font-medium mb-2">End Date *</label>
                      <input type="date" name="endDate" value={form.endDate || ''} readOnly className={`w-full bg-white/10 border rounded-lg px-4 py-3 text-white font-abeze placeholder-gray-400 focus:outline-none transition-colors opacity-70 cursor-not-allowed ${errors.endDate ? 'border-red-400' : 'border-white/20 focus:border-green-400'}`} tabIndex={-1} />
                      {errors.endDate && <p className="text-red-400 text-sm mt-1 font-abeze">{errors.endDate}</p>}
                    </div>
                  </div>
                  <div>
                    <label className="block text-white font-abeze font-medium mb-2">Number of People *</label>
                    <input type="number" name="numberOfPeople" value={form.numberOfPeople || 1} onChange={handleInputChange} min="1" max={packageData.maxGroupSize} className={`w-full bg-white/10 border rounded-lg px-4 py-3 text-white font-abeze placeholder-gray-400 focus:outline-none transition-colors ${errors.numberOfPeople ? 'border-red-400' : 'border-white/20 focus:border-green-400'}`} />
                    {errors.numberOfPeople && <p className="text-red-400 text-sm mt-1 font-abeze">{errors.numberOfPeople}</p>}
                  </div>
                  <div>
                    <label className="block text-white font-abeze font-medium mb-2">Emergency Contact *</label>
                    <input type="tel" name="emergencyContact" value={form.emergencyContact || ''} onChange={handleInputChange} className={`w-full bg-white/10 border rounded-lg px-4 py-3 text-white font-abeze placeholder-gray-400 focus:outline-none transition-colors ${errors.emergencyContact ? 'border-red-400' : 'border-white/20 focus:border-green-400'}`} placeholder="+94 71 123 4567" />
                    {errors.emergencyContact && <p className="text-red-400 text-sm mt-1 font-abeze">{errors.emergencyContact}</p>}
                  </div>
                  <div className="grid md:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-white font-abeze font-medium mb-2">Accommodation Preference</label>
                      <select name="accommodationPreference" value={form.accommodationPreference || 'Standard'} onChange={handleInputChange} className="w-full bg-gray-800/80 border border-white/20 rounded-lg px-4 py-3 text-white font-abeze focus:outline-none focus:border-green-400 transition-colors">
                    ...
                    <div>
                      <label className="block text-white font-abeze font-medium mb-2">Transportation Preference</label>
                      <select name="transportationPreference" value={form.transportationPreference || 'Included'} onChange={handleInputChange} className="w-full bg-gray-800/80 border border-white/20 rounded-lg px-4 py-3 text-white font-abeze focus:outline-none focus:border-green-400 transition-colors">
                  ...
                  <div>
                    <label className="block text-white font-abeze font-medium mb-2">Special Requests</label>
                    <textarea name="specialRequests" value={form.specialRequests || ''} onChange={handleInputChange} rows="3" className="w-full bg-white/10 border border-white/20 rounded-lg px-4 py-3 text-white font-abeze placeholder-gray-400 focus:outline-none focus:border-green-400 transition-colors" placeholder="Any special requirements or requests..." />
                  </div>
                  <div>
                    <label className="block text-white font-abeze font-medium mb-2">Dietary Restrictions</label>
                    <textarea name="dietaryRestrictions" value={form.dietaryRestrictions || ''} onChange={handleInputChange} rows="2" className="w-full bg-white/10 border border-white/20 rounded-lg px-4 py-3 text-white font-abeze placeholder-gray-400 focus:outline-none focus:border-green-400 transition-colors" placeholder="Vegetarian, Vegan, Gluten-free, etc." />
                  </div>
                  <div className="bg-green-600/20 rounded-lg p-4 border border-green-400/30">
                    <div className="flex justify-between items-center mb-2">
                      <span className="text-white font-abeze font-medium">Total Price:</span>
                      <span className="text-2xl font-abeze font-bold text-green-400">LKR {calculateTotalPrice().toLocaleString()}</span>
                    </div>
                    <div className="space-y-1 text-sm">
                      <div className="flex justify-between">
                        <span className="text-gray-300 font-abeze">Base Price:</span>
                        <span className="text-gray-300 font-abeze">LKR {(packageData.price * (form.numberOfPeople || 1)).toLocaleString()}</span>
                      </div>
                      {form.accommodationPreference !== 'Standard' && (
                        <div className="flex justify-between">
                          <span className="text-gray-300 font-abeze">Accommodation Upgrade:</span>
                          <span className="text-green-400 font-abeze">+LKR {(() => {
                            switch (form.accommodationPreference) {
                              case 'Luxury': return (5000 * (form.numberOfPeople || 1)).toLocaleString();
                              case 'Tented Camp': return (2000 * (form.numberOfPeople || 1)).toLocaleString();
                              case 'Eco Lodge': return (3000 * (form.numberOfPeople || 1)).toLocaleString();
                              default: return '0';
                            }
                          })()}</span>
                        </div>
                      ...
                      {form.transportationPreference !== 'Included' && (
                        <div className="flex justify-between">
                          ...
                          <span className="text-green-400 font-abeze">+LKR {(() => {
                            switch (form.transportationPreference) {
                              case 'Private Vehicle': return (3000 * (form.numberOfPeople || 1)).toLocaleString();
                              case 'Shared Vehicle': return (1000 * (form.numberOfPeople || 1)).toLocaleString();
                              default: return '0';
                            }
                          })()}</span>
                        </div>
                  ...
                  <button type="submit" disabled={isSubmitting || (packageData && form.numberOfPeople > packageData.maxGroupSize)} className="w-full bg-green-600 hover:bg-green-700 disabled:bg-gray-600 text-white py-4 rounded-lg font-abeze font-bold transition-colors duration-300">
